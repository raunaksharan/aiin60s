// api/groq.js - Vercel serverless function to proxy Groq API calls
...
- Make the agents feel like a natural next step after the prompts.`;
  ...
  try {
    ...
    if (!groqResponse.ok) {
      ...
      return res.status(502).json({ error: `Groq API error: ${groqResponse.status}`, detail: errText });
    }
    ...
    if (!jsonMatch) {
      return res.status(502).json({ error: 'Invalid JSON response from Groq', raw: content });
    }
